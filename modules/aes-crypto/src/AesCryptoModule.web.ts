...
class EncryptionKey {
  ...
  static async generate(input?: KeySize | Uint8Array | string, encoding?: 'hex' | 'base64'): Promise<EncryptionKey> {
    if (input instanceof Uint8Array || typeof input === 'string') {
      let bytes;
      if (typeof input === 'string') {
        bytes = encoding === 'base64' ?
          base64ToArrayBuffer(input) :
          hexToUintArray(input)
      } else {
        bytes = input;
      }
...
class SealedData {
  ...
  static fromNonceAndCiphertext(iv: SerializableInput, ciphertext: SerializableInput, tag?: SerializableInput | number): SealedData {
  ...
  async ciphertext(options?: { withTag?: boolean, encoding?: 'bytes' | 'base64' }): Promise<Uint8Array | string> {
...
class AesCryptoModule extends NativeModule {
  ...
  async importKey(keyInput: string | Uint8Array, encoding?: 'hex' | 'base64'): Promise<EncryptionKey> {
